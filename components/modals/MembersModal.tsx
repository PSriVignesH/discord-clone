...
import { Check, CircleUserRound, Gavel, Loader2, MoreVertical, Shield, ShieldAlert, ShieldCheck, ShieldQuestion} from 'lucide-react'
...
const MembersModal = () => {
  ...
  const router = useRouter()
   ...
   const isModalOpen= modal.isOpen && modal.type ==="members"
   const {server} =modal.data as {server:ServerWithMembersWithProfiles}
      ...
      const onKick =async(memberId:string)=>{
          ...
          router.refresh()
          modal.onOpen("members",{server:response.data})
           } catch (error) {
            console.log(error);
