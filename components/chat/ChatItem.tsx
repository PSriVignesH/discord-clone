...
import { CircleUserRound, Edit, FileIcon, ShieldAlert, ShieldCheck, Trash} from "lucide-react"
...
const roleIconMap = {
  "GUEST": <CircleUserRound className="h-4 w-4 ml-2 text-green-500"  />,
...
const ChatItem:FC<ChatItemProps>= ({id,content,member,timestamp,fileUrl,deleted,currentMember,isUpdated,socketQuery,socketUrl}) => {
  ...
  useEffect(()=>{
    const handleKeyKown =(event:any)=>{
      if(event.key === "Escape" || event.keyCode === 27){
        setIsEditing(false)
      }
    }

    window.addEventListener("keydown",handleKeyKown)
    return ()=> window.removeEventListener("keydown",handleKeyKown)
  },[])
      ...
      </div>
      {
        canDeleteMessage && (
          <div className="hidden group-hover:flex  items-center gap-x-2 absolute p-1 -top-2 right-5 bg-white dark:bg-zinc-800 border rounded-sm">
            ...
            <Trash  className="cursor-pointer ml-auto w-4 h-4 text-zinc-500 hover:text-zinc-600 dark:hover:text-zinc-300 transition" onClick={()=>modal.onOpen("deleteMessage",{
               apiUrl: `${socketUrl}/${id}`,
               query: socketQuery,
            })}/>
            </ActionTooltip>
